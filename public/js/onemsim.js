...
//     // var mySocket = socketFactory({
//     //     ioSocket: null // The ws connection should be done after api/start
//     // })
//     return mySocket;
// });
...
ONEmSimModule.controller('mainController', [
    ...
    'dateFilter',
    '$rootScope',
    function($scope, $http, SmsHandler, DataModel, dateFilter, $rootScope) {

        console.log("[MN]: mainController initialising");
        SmsHandler.start({}, function(startResponse) {
            // console.log(Socket);

            socketIO = io.connect('ws://' + window.location.host);
            socketIO.on('connect', function () {
                socketIO.on('error', function(data) {
                    console.log(data);
                });

                socketIO.on('MT SMS', function(data) {
                    $scope.theData = data;
                ...
                $scope.smsInput = function() {
                    ...
                    socketIO.emit('MO SMS', $scope.smsText);
                ...
                socketIO.emit('thePath',$rootScope.myLocation);
            });
        });
    }
]);
